...
st.subheader("Cuándo me toca Santa Cruz?")
...
#     all_tenants = df['SantaCruz'].dropna().unique().tolist()
#     all_tenants.insert(0, "Todos")  # Add "All" option at the beginning
#     selected_tenant = cols[0].selectbox('Inquilino', all_tenants, key='selected_tenant')
    
#     if selected_tenant == "Todos":
#         df_filtered = df
#     else:
#         df_filtered = df[df['SantaCruz'] == selected_tenant]
    
#     # Default to selecting all years
#     all_years = df_filtered['Año'].unique().astype(int).tolist()
#     all_years.insert(0, "Todos")  # Add "All" option at the beginning
#     selected_year = cols[1].selectbox('Año', all_years, key='selected_year')
